...
class ResourceSharedNotesViewModel {
  final NoteRemoteDataSource noteDataSource;

  ResourceSharedNotesViewModel({required this.noteDataSource}) {
  ...
  final ValueNotifier<List<NoteEntity>> notes =
      ValueNotifier<List<NoteEntity>>([]);
  final ValueNotifier<String> searchQuery = ValueNotifier<String>('');
  late final ValueNotifier<List<NoteEntity>> filteredNotes;
  ...
  Future<void> loadSharedNotes(int idRecurso) async {
    ...
    try {
      final loadedNotes =
          await noteDataSource.getSharedNotesByResource(idRecurso);
